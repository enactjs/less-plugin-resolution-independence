	...
	ResolutionIndependencePlugin.prototype = {
		...
		*/
		configure: function (opts) {
			this._baseSize = (opts && 'baseSize' in opts) ? opts.baseSize : this._baseSize;
			this._riUnit = (opts && 'riUnit' in opts) ? opts.riUnit : this._riUnit;
			this._unit = (opts && 'unit' in opts) ? opts.unit : this._unit;
			this._absoluteUnit = (opts && 'absoluteUnit' in opts) ? opts.absoluteUnit : this._absoluteUnit;
			this._minUnitSize = (opts && 'minUnitSize' in opts) ? opts.minUnitSize : this._minUnitSize;
			this._minSize = (opts && 'minSize' in opts) ? opts.minSize : this._minSize;
			this._precision = (opts && 'precision' in opts) ? opts.precision : this._precision;
		...
		*/
		visitRule: function (node) {
			...
			if (Array.isArray(ruleNode.args)) {
				this.processRuleNodes(ruleNode.args);
			}
		...
		*
		* @param {Array} ruleNodes - An array of LESS rule nodes we wish to parse and convert.
		* @private
		*/
		processRuleNodes: function (ruleNodes) {
			for (var idx = 0; idx < ruleNodes.length; idx++) {
				this.updateNode(ruleNodes[idx]);
			}
		},
